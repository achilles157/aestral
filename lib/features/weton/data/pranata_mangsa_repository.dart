...
final pranataMangsaListProvider = FutureProvider<List<PranataMangsaModel>>((
  ref,
) async {
  final String jsonString = await rootBundle.loadString(
    'assets/weton/pranata_mangsa.json',
  );
  final List<dynamic> jsonList = json.decode(jsonString);
  return jsonList.map((json) => PranataMangsaModel.fromJson(json)).toList();
});

final pranataMangsaRepositoryProvider = Provider(
  (ref) => PranataMangsaRepository(ref),
);
