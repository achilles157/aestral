...
class WetonCalculatorScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<WetonCalculatorScreen> createState() =>
      _WetonCalculatorScreenState();
}
...
class _WetonCalculatorScreenState extends ConsumerState<WetonCalculatorScreen> {
  ...
  Future<void> _handleCalculate() async {
    ...
    try {
      ...
      final targetPranataId = WetonUtils.calculatePranataMangsaId(
        DateTime.now(),
      );
      ...
      if (mounted) {
        setState(() {
          _dailyInsightData = {
            'birthWeton': {'pranataMangsaId': birthPranataId},
            'targetWeton': {'pranataMangsaId': targetPranataId},
            'daily': {
              'sisaBagi': sisaBagi,
              'fase': sisaBagi == 1
                  ? 'Sandang'
        ...
        SnackBar(
          content: Row(
            children: [
              ...
              Text(
                success
                    ? 'Profil berhasil disimpan ke takdir Anda!'
                                      ...
                                      OutlinedButton.icon(
                                        onPressed: () => Navigator.of(context).push(
                                          MaterialPageRoute<void>(
                                            builder: (_) =>
                                                const WetonCompatibilityScreen(),
                                          ),
                                        ),
