...
class BaziTenGodsWidget extends StatelessWidget {
        ...
        children: [
          ...
          Row(
            children: columns.map((col) {
              ...
              final String godId =
                  BaziUtils.getTenGodId(dmIdx, col.pillar!.stemIndex);
              final Map<String, dynamic>? godEntry = godsData?.firstWhere(
                (g) => g['id'] == godId,
                orElse: () => <String, dynamic>{},
              );
              final bool canTap =
                  godEntry != null && godEntry.isNotEmpty;

              return Expanded(
                child: GestureDetector(
                  onTap: godEntry != null && godEntry.isNotEmpty
                      ? () => _showGodDetail(
                          context, godId, godEntry, col.pillar!, elementColor)
                      : null,
                  child: _GodChip(
                    col: col,
                    dmStemIndex: dmIdx,
                    ...
                    namePendekOverride: godEntry?['nama_pendek'] as String?,
                  ),
                ),
              );
            }).toList(),
  ...
  void _showGodDetail(
    ...
    final String hanzi = data['hanzi'] as String? ?? _kTenGodNames[godId]?.$1 ?? '?';
    final String nameId = data['nama_pendek'] as String? ?? _kTenGodNames[godId]?.$2 ?? godId;
    final Color color =
        kBaziElementColors[pillar.element] ?? elementColor;
    final String fokus = data['fokus_utama'] as String? ?? '';
    final String interp =
        data['interpretasi_psikologis'] as String? ?? '';
...
class _GodChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final String nameId = namePendekOverride ?? _kTenGodNames[godId]?.$2 ?? godId;
    final Color color =
